...
router.get('/characters', async (req, res) => {
    const { name, status, gender } = req.query;
    if (name || status || gender) {
        const characters = await Character.find({ Name: new RegExp(name, 'i'), Status: new RegExp (status, 'i'), Gender: new RegExp
        (gender, 'i') });
...
router.get('/characters/random', async (req, res) => {
    ...
    try {
        if (limit) {
            const characters = await Character.aggregate([{ $sample: { size: parseInt(limit) } }]);
